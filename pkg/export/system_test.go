	...
	tests := []struct {
		...
		},
		{
			...
			setup: func(s *System) error {
				...
				supportedDrivers[dapr.Name] = dapr.FakeConn
				return dapr.FakeConn.CreateConnection(ctx, "conn1", map[string]interface{}{"component": "pubsub"})
			},
			wantErr: false,
		},
		{
		...
		},
		{
			...
			setup: func(s *System) error {
				...
				return testdriver.FakeConn.CreateConnection(ctx, "conn4", "config4")
			},
			wantErr: false,
		},
	}
	...
	tests := []struct {
		...
		},
		{
			...
			setup: func(s *System) {
				...
				_ = testdriver.FakeErrConn.CreateConnection(ctx, "conn3", "config3")
			},
